...
func findNonCompositionDerivative(parent *jsonValue, fault faultTarget, s *search) (*jsonValue, bool, error) {
	if fault.obligation.rule == oracleRuleRequired {
		return findRequiredDerivative(parent, fault, s.model)
	}
	...
	case oracleRuleType:
		return findTypeDerivative(parent, fault, node, s.model)
	case oracleRuleEnum:
		return findEnumDerivative(parent, fault, node, s.model)
...
func findTypeDerivative(
	...
	for _, kind := range canonicalJSONKinds() {
		...
		if derivative, found, err := firstReplacementDerivative(parent, fault, candidates, model); err != nil || found {
	...
	complete, err := s.walkActiveNumberRules(
		container, occurrence, fault.pins, nil, func(candidate *jsonValue) (bool, error) {
			selected, matched, selectErr := firstReplacementDerivative(
				parent, fault, []*jsonValue{candidate}, s.model,
			)
...
func findStringDerivative(parent *jsonValue, fault faultTarget, s *search) (*jsonValue, bool, error) {
	...
	return firstReplacementDerivative(parent, fault, []*jsonValue{candidate}, s.model)
}
...
func findArrayCountDerivative(
	...
	for _, path := range paths {
		...
		if !fits || count > uint64(maxInt()) {
			return nil, false, errors.New("schematest: array fault count does not fit memory index")
		}
		...
		if fault.obligation.rule == oracleRuleMinItems {
			...
			if desired == maxInt() {
				return nil, false, errors.New("schematest: array fault count overflows memory index")
			}
		...
		for len(array.array) < desired {
			item, itemErr := validArrayItem(node, occurrence, fault.pins, array, s)
			if itemErr != nil {
...
func validArrayItem(
	...
	if len(array.array) > 0 {
		return copyJSONValue(array.array[0], make(map[*jsonValue]*jsonValue))
	}
	...
	complete, err := s.walkNode(
		node.items, itemOccurrence, pins, rowSearchContext{}, func(value *jsonValue) (bool, error) {
			result := evaluateNode(node.items, value, itemOccurrence)
			if !result.valid {
	...
	if !complete {
		return nil, errors.New("schematest: array item fault repair has no valid value")
	}
...
func findObjectCountDerivative(
	...
	for _, path := range paths {
		...
		if !fits || count > uint64(maxInt()) {
			return nil, false, errors.New("schematest: object fault count does not fit memory index")
		}
		...
		if fault.obligation.rule == oracleRuleMinProperties {
			...
			desired--
			removeOptionalMembers(object.object, node.required, desired)
		} else {
			if desired == maxInt() {
				return nil, false, errors.New("schematest: object fault count overflows memory index")
			}
...
func removeOptionalMembers(members map[string]*jsonValue, required []string, desired int) {
	requiredSet := make(map[string]bool, len(required))
	for _, name := range required {
		requiredSet[name] = true
	}

	names := sortedObjectNames(members)
	for index := len(names) - 1; index >= 0 && len(members) > desired; index-- {
		if !requiredSet[names[index]] {
			delete(members, names[index])
		}
	}
}
...
func addValidObjectMembers(
	...
	for _, name := range sortedSchemaPropertyNames(node.properties) {
		...
		value, err := firstValidNodeValue(property, propertyOccurrence, pins, s)
		if err != nil {
			return err
		}
	...
	for len(object.object) < desired &&
		...
		if node.additionalProperties != nil {
			...
			value, err = firstValidNodeValue(node.additionalProperties, additionalOccurrence, pins, s)
			if err != nil {
				return err
			}
		}
...
func firstValidNodeValue(
	...
	if !complete {
		return nil, errors.New("schematest: object fault repair has no valid property value")
	}

	return found, nil
}

func findRequiredDerivative(parent *jsonValue, fault faultTarget, model *schemaModel) (*jsonValue, bool, error) {
	...
	for _, path := range matchingValuePaths(parent, parentPointer) {
		...
		matched, err := derivativeHasClosure(model, candidate, fault.closure)
		if err != nil || matched {
...
func findAdditionalPropertyDerivative(
	...
	for _, path := range matchingValuePaths(parent, parentPointer) {
		...
		object.object[name] = &jsonValue{kind: jsonBoolean}
