...
var exactOwnershipAllowlist = exactOwnershipRows(
	...
	ownershipRowGroup{form: ownershipMachineState, lifetime: ownershipAttemptLifetime, rows: []string{
		...
		"github.com/djosh34/klopt/pkg/schematest.arrayCombinationRankCursor.length|int",
		"github.com/djosh34/klopt/pkg/schematest.arrayCombinationRankCursor.next|int",
		"github.com/djosh34/klopt/pkg/schematest.arrayCombinationRankCursor.rank|uint64",
		"github.com/djosh34/klopt/pkg/schematest.arrayCombinationRankCursor.remaining|int",
		...
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.childFiniteEnd|uint64",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.childRank|uint64",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.context|rowSearchContext",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.items|rowSchemaConjunction",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.position|uint64",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.projection|uint64",
		"github.com/djosh34/klopt/pkg/schematest.beyondArrayCursor.requirements|[]requirement",
		...
		"github.com/djosh34/klopt/pkg/schematest.directRankTupleDecoder.dimensions|uint64",
		"github.com/djosh34/klopt/pkg/schematest.directRankTupleDecoder.dimension|uint64",
		...
		"github.com/djosh34/klopt/pkg/schematest.liveProjectionFrontier.cursor|*rowProjectionCursor",
		"github.com/djosh34/klopt/pkg/schematest.liveProjectionFrontier.exhausted|bool",
		"github.com/djosh34/klopt/pkg/schematest.liveProjectionFrontier.finiteSize|uint64",
		"github.com/djosh34/klopt/pkg/schematest.liveProjectionFrontier.ordinal|uint64",
	...
	ownershipRowGroup{form: ownershipMachineState, lifetime: ownershipCallLifetime, rows: []string{
		...
		"github.com/djosh34/klopt/pkg/schematest.ecmaPatternParser.lowSurrogateContinuation|bool",
		"github.com/djosh34/klopt/pkg/schematest.ecmaPatternParser.nesting|int",
		"github.com/djosh34/klopt/pkg/schematest.ecmaPatternParser.nodeCount|int",
		"github.com/djosh34/klopt/pkg/schematest.ecmaPatternParser.position|int",
		"github.com/djosh34/klopt/pkg/schematest.ecmaPatternParser.units|[]uint16",
		"github.com/djosh34/klopt/pkg/schematest.strictJSONParser.position|int",
		"github.com/djosh34/klopt/pkg/schematest.strictJSONParser.source|[]byte",
	}},
	ownershipRowGroup{form: ownershipCurrentValue, lifetime: ownershipAttemptLifetime, rows: []string{
...
func TestExactOwnershipGuardDiscoversDynamicCalleeOwners(t *testing.T) {
	t.Parallel()

	source := `package schematest
		type runner interface { run() }
		type implementation struct { hidden []string }
		func (*implementation) run() {}
		type storedOwner struct { hidden []byte }
		var storedFunction = stored
		func stored() { _ = new(storedOwner) }
		func Build(value runner) { value.run(); storedFunction() }
	`

	violations := exactOwnershipViolations(parseGuardPackage(t, map[string]string{"guard.go": source}), nil)
	require.Contains(t, violations, "unlisted owner field: "+modulePath+"/pkg/schematest.implementation.hidden")
	require.Contains(t, violations, "unlisted owner field: "+modulePath+"/pkg/schematest.storedOwner.hidden")
}
...
func TestExactOwnershipGuardRejectsMisleadingCarrierRows(t *testing.T) {
	...
	tests := []struct {
		...
		{typeName: "*jsonValue", field: "value"},
		{typeName: "struct{ nested *jsonValue }", field: "value"},
	}
	for _, test := range tests {
...
func TestSingularCurrentOwnershipRequiresClearBeforeContinuation(t *testing.T) {
	...
	instancePath := `package schematest
		type jsonValue struct{}; type state struct { current *jsonValue }
		func Build(yield func()) { first, second := new(state), new(state); first.current = new(jsonValue); second.current = new(jsonValue); second.current = nil; yield() }
	`
	require.NotEmpty(t, exactOwnershipViolations(parseGuardPackage(t, map[string]string{"guard.go": instancePath}), allowed))

	badPaths := []string{
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func advance() int { return 1 }; func Build() { active := new(state); active.current = new(jsonValue); var result = advance(); active.current = nil; _ = result }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func append() {}; func Build() { active := new(state); active.current = new(jsonValue); append(); active.current = nil }`,
		...
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := new(state); switch 1 { case 1: active.current = new(jsonValue) }; yield() }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := new(state); var value any = 1; switch value.(type) { case int: active.current = new(jsonValue) }; yield() }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := new(state); select { default: active.current = new(jsonValue) }; yield() }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := new(state); for { active.current = new(jsonValue); break }; yield() }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := new(state); for range 1 { active.current = new(jsonValue) }; yield() }`,
		`package schematest; type jsonValue struct{}; type state struct { current *jsonValue }; func Build(yield func()) { active := &state{current: new(jsonValue)}; yield(); _ = active }`,
	}
...
func TestCurrentOwnershipRejectsCollectionsAndCallLifetimeEscape(t *testing.T) {
	...
	collectionSource := `package schematest
		type jsonValue struct{}; type state struct { current []*jsonValue }
		func Build() { _ = new(state) }
	`
	collectionAllowed := map[string]ownershipAllowance{
		prefix + "state.current": {typeName: "[]*jsonValue", form: ownershipCurrentValue, lifetime: ownershipAttemptLifetime},
	}
	require.NotEmpty(t, exactOwnershipViolations(parseGuardPackage(t, map[string]string{"guard.go": collectionSource}), collectionAllowed))

	cacheSource := `package schematest
		type jsonValue struct{}; type cacheKey struct { value *jsonValue }; type evaluationContext struct { cache map[cacheKey]bool }
		func Build() evaluationContext { context := evaluationContext{cache: make(map[cacheKey]bool)}; return context }
	`
	cacheAllowed := map[string]ownershipAllowance{
		prefix + "cacheKey.value":          {typeName: "*jsonValue", form: ownershipCurrentValue, lifetime: ownershipCallLifetime},
		prefix + "evaluationContext.cache": {typeName: "map[cacheKey]bool", form: ownershipCurrentValue, lifetime: ownershipCallLifetime},
	}
	require.NotEmpty(t, exactOwnershipViolations(parseGuardPackage(t, map[string]string{"guard.go": cacheSource}), cacheAllowed))
}
...
//nolint:cyclop,gocognit,gocyclo // Package, SSA value, and structural field discovery form one ownership graph.
func buildOwnershipFields(guardPackage *sourceGuardPackage) map[string]*types.Var {
	...
	for _, file := range guardPackage.files {
		for _, declaration := range file.Decls {
			...
			if object, objectOK := guardPackage.info.Defs[function.Name].(*types.Func); objectOK {
	...
	for _, file := range guardPackage.files {
		ast.Inspect(file, func(node ast.Node) bool {
			...
			for _, value := range values {
				...
				object, ok := guardPackage.info.Uses[identifier].(*types.Func)
				if !ok {
					continue
				}

				if function := ssaPackage.Func(object.Name()); function != nil {
					functions[function] = true
				}

				if declaration := declarations[object]; declaration != nil {
					ast.Inspect(declaration, func(child ast.Node) bool {
						expression, expressionOK := child.(ast.Expr)
						if expressionOK {
							collectNamedOwnerTypes(guardPackage.info.TypeOf(expression), owners)
						}
...
func validOwnershipAllowance(
	...
	validLifetime := map[ownershipForm]map[ownershipLifetime]bool{
		...
		ownershipCurrentValue: {
			ownershipAttemptLifetime: true, ownershipCallLifetime: true, ownershipBeforeContinuation: true,
		},
	}
	...
	containsJSONValue := containsRuntimeJSONCarrier(
		owned, packageObjectType(guardPackage, "jsonValue"), make(map[types.Type]bool),
	)
	if containsJSONValue && allowance.form != ownershipAuthoredValue && allowance.form != ownershipCurrentValue {
		return false
	}

	if containsJSONValue && allowance.form == ownershipCurrentValue && allowance.lifetime == ownershipAttemptLifetime {
...
//nolint:cyclop // Every direct runtime carrier wrapper is classified explicitly.
func containsRuntimeJSONCarrier(owned, wanted types.Type, visiting map[types.Type]bool) bool {
	...
	case *types.Pointer:
		element := types.Unalias(typed.Elem())
		if named, ok := element.(*types.Named); ok && !sameGuardType(named, wanted) {
			return false
		}

		return containsRuntimeJSONCarrier(element, wanted, visiting)
	}

	switch typed := owned.Underlying().(type) {
	case *types.Slice:
		return containsRuntimeJSONCarrier(typed.Elem(), wanted, visiting)
	case *types.Array:
		return containsRuntimeJSONCarrier(typed.Elem(), wanted, visiting)
	case *types.Map:
		return containsRuntimeJSONCarrier(typed.Key(), wanted, visiting) ||
			containsRuntimeJSONCarrier(typed.Elem(), wanted, visiting)
	case *types.Struct:
		for index := range typed.NumFields() {
			if containsRuntimeJSONCarrier(typed.Field(index).Type(), wanted, visiting) {
				return true
			}
		}
	}
...
func currentValueLifecycleCoverage(
	guardPackage *sourceGuardPackage,
	allowed map[string]ownershipAllowance,
...
func currentValueLifecycleAnalysis(
	...
	for _, file := range guardPackage.files {
		for _, declaration := range file.Decls {
			...
			live := make(map[ownershipLiveValue]bool)
			inspectOwnershipStatements(guardPackage, function.Body.List, watched, live, &violations)

			for value := range live {
				violations = append(violations, "current value survives function return: "+watched[value.field])
			}
		}
	}
...
func ownershipTypeContainsNamed(owned types.Type, currentPackage *types.Package, name string, seen map[types.Type]bool) bool {
	if owned == nil {
		return false
	}
	...
	if named, ok := owned.(*types.Named); ok && named.Obj().Pkg() == currentPackage && named.Obj().Name() == name {
		return true
	}

	switch typed := owned.Underlying().(type) {
	case *types.Pointer:
		return ownershipTypeContainsNamed(typed.Elem(), currentPackage, name, seen)
	case *types.Array:
		return ownershipTypeContainsNamed(typed.Elem(), currentPackage, name, seen)
	case *types.Slice:
		return ownershipTypeContainsNamed(typed.Elem(), currentPackage, name, seen)
	case *types.Map:
		return ownershipTypeContainsNamed(typed.Key(), currentPackage, name, seen) ||
			ownershipTypeContainsNamed(typed.Elem(), currentPackage, name, seen)
	}
...
func inspectOwnershipStatements(
	...
	live map[ownershipLiveValue]bool,
	violations *[]string,
) {
	for _, statement := range statements {
		markCompositeOwnership(guardPackage, statement, watched, live)
		...
		if assignment, ok := statement.(*ast.AssignStmt); ok {
			for index, left := range assignment.Lhs {
				...
				value := ownershipLiveValue{receiver: ownershipReceiverPath(selector.X), field: field}

				live[value] = !isNilExpression(assignment.Rhs[index])
				if !live[value] {
					delete(live, value)
				}
			}
		...
		case *ast.AssignStmt:
			if ownershipContinuationCall(guardPackage, typed) {
		...
		case *ast.IfStmt:
			if ownershipContinuationCall(guardPackage, typed.Init) || ownershipContinuationCall(guardPackage, typed.Cond) {
				appendLiveOwnershipViolations(live, watched, "continuation", violations)
			}
		...
		case *ast.SwitchStmt:
			if ownershipContinuationCall(guardPackage, typed.Init) || ownershipContinuationCall(guardPackage, typed.Tag) {
				appendLiveOwnershipViolations(live, watched, "continuation", violations)
			}

			join := make(map[ownershipLiveValue]bool)
			hasDefault := false
		...
		case *ast.TypeSwitchStmt:
			if ownershipContinuationCall(guardPackage, typed.Init) || ownershipContinuationCall(guardPackage, typed.Assign) {
				appendLiveOwnershipViolations(live, watched, "continuation", violations)
			}
		...
		case *ast.SelectStmt:
			...
			for _, clause := range typed.Body.List {
				...
				branchLive := maps.Clone(live)
				if ownershipContinuationCall(guardPackage, communication.Comm) {
		...
		case *ast.ForStmt:
			...
			for value := range nested {
				*violations = append(*violations, "current value survives retry: "+watched[value.field])
				live[value] = true
			}
		case *ast.RangeStmt:
			nested := maps.Clone(live)
			...
			for value := range nested {
				*violations = append(*violations, "current value survives sibling: "+watched[value.field])
				live[value] = true
			}
		}
	}
...
func markCompositeOwnership(
	...
	ast.Inspect(node, func(child ast.Node) bool {
		...
		for _, element := range literal.Elts {
			...
			field, ok := guardPackage.info.Uses[identifier].(*types.Var)
			if ok && watched[field] != "" && !isNilExpression(keyValue.Value) {
				live[ownershipLiveValue{receiver: "composite", field: field}] = true
			}
		}
...
func appendLiveOwnershipViolations(
	...
	for value := range live {
		*violations = append(*violations, "current value survives "+boundary+": "+watched[value.field])
	}
}

func ownershipReceiverPath(expression ast.Expr) string {
	switch typed := expression.(type) {
	case *ast.Ident:
		return typed.Name
	case *ast.SelectorExpr:
		return ownershipReceiverPath(typed.X) + "." + typed.Sel.Name
	case *ast.ParenExpr:
		return ownershipReceiverPath(typed.X)
	case *ast.StarExpr:
		return ownershipReceiverPath(typed.X)
	default:
		return "unknown"
...
func ownershipContinuationCall(guardPackage *sourceGuardPackage, node ast.Node) bool {
	if node == nil {
		return false
	}
	...
	ast.Inspect(node, func(child ast.Node) bool {
		...
		if identifier, identifierOK := call.Fun.(*ast.Ident); identifierOK {
			if _, builtin := guardPackage.info.Uses[identifier].(*types.Builtin); builtin {
				return false
			}
		}
