...
func TestEvaluationRecordsCacheHitKeepsEveryFact(t *testing.T) {
	...
	require.Equal(t, []string{"type"}, applicableRules(first.applicable))
	require.Equal(t, []string{"type"}, applicableRules(second.applicable))
	require.Equal(t, []string{"string"}, observedLevels(first.observed))
	require.Equal(t, []string{"string"}, observedLevels(second.observed))
