...
//nolint:cyclop // Exhaustive product exploration keeps every resource check at its allocation site.
func buildProductGraph(
	...
	graph.states = append(graph.states, productState{
		accepting: productAccepts(initial, machines, requirements, minLength, maxLength),
	})

	for current := 0; current < len(tuples); current++ {
		tuple := tuples[current]
		if maxLength != nil && tuple.length == *maxLength {
			continue
		}
		...
		for value := range asciiAlphabetSize {
			next := productTuple{
				...
				length:   nextLength(tuple.length, minLength, maxLength),
			}
			...
			if !ok {
				...
				graph.states = append(graph.states, productState{
					accepting: productAccepts(next, machines, requirements, minLength, maxLength),
				})
			}
