//nolint:cyclop,gocognit,godoclint,nlreturn,wsl_v5 // Fake validators are clearest as compact fixtures.
...
func TestCheckJSONRequestBodyFindsBuggyValidatorsByKeywordFamily(t *testing.T) {
	...
	//nolint:paralleltest // Subprocesses run serially to avoid competing race-instrumented test binaries.
	for _, family := range families {
		t.Run(family, func(t *testing.T) {
			...
			require.Error(t, err)
			require.Contains(t, string(output), validatorBugFixtures()[family].failure)
		})
	}
}
